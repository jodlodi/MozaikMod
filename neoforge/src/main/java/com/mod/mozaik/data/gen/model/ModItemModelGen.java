...
public class ModItemModelGen extends ItemModelProvider {
	...
	protected void registerModels() {
		ModBlocks.MORTARS.forEach(supplier -> {
			this.withExistingParent(supplier.id().getPath(), Constants.prefix("block/mortar"))
	...
	private void toBlock(Block b) {
		this.toBlockModel(b, BuiltInRegistries.BLOCK.getKey(b).getPath());
	}
	...
	private ItemModelBuilder buildItem(String name, String parent, int emissivity, ResourceLocation... layers) {
		ItemModelBuilder builder = this.withExistingParent(name, parent);
