...
@NullMarked
public record OldPolyomino(List<Tessera.PlacedTessera> placedTessera, TesseraMaterial material, long seed) {
	public static OldPolyomino EMPTY = new OldPolyomino(List.of(), TesseraMaterial.STONE, 0L);

	private OldPolyomino(List<Tessera.PlacedTessera> placedTessera, int ordinal, long seed) {
		this(placedTessera, TesseraMaterial.values()[ordinal], seed);
	}

	public OldPolyomino rebuild(TesseraMaterial material) {
		return new OldPolyomino(this.placedTessera(), material, MortarScreen.randomSeed());
	}

	public OldPolyomino copy() {
		return new OldPolyomino(this.placedTessera, this.material, this.seed);
	}

	public static final Codec<OldPolyomino> CODEC = RecordCodecBuilder.create((recordCodecBuilder) -> recordCodecBuilder.group(
			Tessera.PlacedTessera.CODEC.listOf().fieldOf("placedPolyomino").forGetter(OldPolyomino::placedTessera),
			Codec.INT.fieldOf("material").forGetter(material -> material.material.ordinal()),
			Codec.LONG.fieldOf("seed").forGetter(OldPolyomino::seed)
	).apply(recordCodecBuilder, OldPolyomino::new));
	...
	public record PlacedPolyomino(OldPolyomino polyomino, int x, int y) {
		public static final Codec<PlacedPolyomino> CODEC = RecordCodecBuilder.create((recordCodecBuilder) -> recordCodecBuilder.group(
				OldPolyomino.CODEC.fieldOf("polyomino").forGetter(PlacedPolyomino::polyomino),
	...
	public static class Builder {
		...
		public OldPolyomino build(TesseraMaterial material, long seed) {
			int x = 0;
			int y = 0;
			...
			return new OldPolyomino(placedTessera, material, seed);
		}
